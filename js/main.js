...
function checkEmail(str) {
  return (/^[a-z-.]+@[a-z]+\.[a-z]{2,3}(\.[a-z]{2,3})?$/).test(str);
}
...
function checkDomainUrl(str) {
  return (/^(http|https)\:\/\/([a-z-_0-9].?)+\.[a-z]+(\.[a-z]{2,3})?(\.[a-z]{2,3})?$/).test(str);
}
...
function createLinksFromDomains(str) {
  let result = str.match(/(http|https)\:\/\/([a-z-_0-9].?)+\.[a-z]+(\.[a-z]{2,3})?(\.[a-z]{2,3})?/gi);
  result.forEach((t1) => str = str.replace(t1,'<a href="' + t1 + '" target="_blank">' + t1.replace(/http:\/\/|https:\/\//, '') + '</a>'));
  return str;
}
